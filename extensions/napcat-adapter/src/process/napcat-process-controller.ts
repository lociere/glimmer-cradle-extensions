...
export class NapcatProcessController {
  ...
  private resolveWindowsDirectLaunchSpec(packageDir: string, workDir: string): LaunchSpec | null {
    ...
    if (!plan) {
      ...
      this.recoveryActions = [
        '确认 data/packages/extensions/napcat-adapter/napcat 指向 OneKey 根目录，且包含 NapCatWinBootMain.exe、QQ.exe 与 versions/<version>/resources/app/napcat/napcat.mjs。',
...
export function buildNapcatStartupRecoveryActions(
  ...
  const actions = [
    ...
    '检查 data/state/extensions/napcat-adapter/napcat/config/webui.json 与 onebot11 配置是否仍指向摇篮声明的端口。',
  ];
  if (hasConflictingQq) {
    actions.unshift(
