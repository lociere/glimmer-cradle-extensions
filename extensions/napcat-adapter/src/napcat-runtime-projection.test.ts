...
function processSnapshot(overrides: Partial<NapcatProcessSnapshot>): NapcatProcessSnapshot {
  return {
    ...
    cwd: 'D:/data/packages/extensions/napcat-adapter/napcat',
    workDir: 'D:/data/state/extensions/napcat-adapter/napcat',
    packageDir: 'D:/data/packages/extensions/napcat-adapter/napcat',
    bootstrap: true,
    ready: false,
    recoveryActions: [],
    ...overrides,
...
describe('NapCat runtime projection', () => {
  ...
  it('does not treat a personal QQ process as a conflict for packaged NapCat QQ', () => {
    expect(isQqProcessConflict(
      ...
      'D:/glimmer/data/packages/extensions/napcat-adapter/napcat/QQ.exe',
      'packaged',
    )).toBe(false);
  });
